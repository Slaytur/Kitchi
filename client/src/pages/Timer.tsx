...
class Timer extends Component<Record<string, unknown>, TimerState> {
    ...
    startTimer = (): void => {
        if (!this.state.isRunning) {
            if (this.state.isCountingDown) this.timer = setInterval(this.countdown, 1000);
            else this.timer = setInterval(this.updateTimer, 1000);
    ...
    updateTimer = (): void => {
        ...
        if (seconds < 59) this.setState({ seconds: seconds + 1 });
    ...
    countdown = (): void => {
        ...
        else if (seconds === 0) this.setState({ minutes: minutes - 1, seconds: 59 });
            ...
            <div className="tw-bg-[#aadc9f7b] tw-p-4 tw-rounded-lg tw-shadow-md">
              ...
              :
                ...
                <div className="tw-flex tw-place-content-center tw-flex-wrap tw-space-x-4">
                    <button className={`tw-bg-[#2f7832] tw-text-white tw-px-4 tw-py-2 tw-rounded ${isRunning ? `tw-opacity-50 tw-cursor-not-allowed` : ``}`} onClick={this.startTimer} disabled={isRunning}>Start</button>
                    <button className={`tw-bg-[#b13939] tw-text-white tw-px-4 tw-py-2 tw-rounded ${!isRunning ? `tw-opacity-50 tw-cursor-not-allowed` : ``}`} onClick={this.stopTimer} disabled={!isRunning}>Stop</button>
                    <button className="tw-bg-[#6cb55c] tw-text-white tw-px-4 tw-py-2 tw-rounded" onClick={this.resetTimer}>Reset</button>
                    <button className="tw-bg-[#d1823ef1] tw-text-white tw-px-4 tw-py-2 tw-rounded" onClick={this.toggleTimerType}>Toggle Timer Type</button>
                </div>
                ...
                Set Seconds:
                    ...
                    <button className="tw-bg-[#2f7832] tw-text-white tw-bg-gree tw-px-4 tw-py-2 tw-rounded" onClick={this.applySetTime}>Apply Set Time</button>
                </div>
            </div>
        );
    }
